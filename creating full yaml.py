...
# Load the Excel file
df = pd.read_excel(
    r"D:\Flask-Api\RE_Wind Route\Data\1. Ph 12 Kallam_Mastersheet_with MEDA_161024 18-10-2024.xlsx",
    skiprows=1,
)
# Load the Excel file
df = pd.read_excel(
    r"F:\Route optimisation RE\interarray\notebooks\Data\1. Ph 12 Kallam_Mastersheet_with MEDA_161024 18-10-2024 (1).xlsx",
    skiprows=1,
)
...
# --- Configuration ---
CSV_PATH = Path(
    r"D:\Flask-Api\RE_Wind Route\Final_setup\converted_wind_locations.csv"
...
# --- CONFIGURATION ---
TURBINES_YAML = Path(
    r"D:\Flask-Api\RE_Wind Route\Final_setup\turbines2.yaml"
)  # path to your turbines.yaml
EXTENTS_YAML = Path(
    r"D:\Flask-Api\RE_Wind Route\Final_setup\extents.yaml"
)  # path where extents.yaml will be saved
...
def decimal_to_dms(lat, lon):
    ...
    def _to_dms(dec, is_lat):
        hemi = (
            "N"
            if (is_lat and dec >= 0)
            else "S" if is_lat else "E" if dec >= 0 else "W"
        )
        dec_abs = abs(dec)
        deg = int(dec_abs)
        mins = (dec_abs - deg) * 60
...
for ln in sub_list:
    ...
    sub_lines.append(
        f"  [{name}] {dec_to_dms(lat_val, True)} {dec_to_dms(lon_val, False)}"
    )
substations_block = "|-\n" + "\n".join(sub_lines)
...
for i, row in df.iterrows():
    ...
    turbine_lines.append(
        f"  {code} {dec_to_dms(row['Latitude'], True)} {dec_to_dms(row['Longitude'], False)}"
    )
turbines_block = "|-\n" + "\n".join(turbine_lines)
...
with open(
    r"D:\Flask-Api\RE_Wind Route\Final_setup\site_full_block_style.yaml",
    "r",
    encoding="utf-8",
) as f:
...
clean_blocks = [blk for idx, blk in enumerate(obs_blocks) if idx not in bad_idxs]

# 5) Write cleaned YAML
data["OBSTACLES"] = clean_blocks
with open(
    r"D:\Flask-Api\RE_Wind Route\Final_setup\final_Serentica_clear.yaml",
    "w",
    encoding="utf-8",
) as f:
...
session_dirs = sorted([d for d in template_dir.iterdir() if d.is_dir()], reverse=True)
if not session_dirs:
    raise RuntimeError("No session directories found in Template")

combined_yaml_path = session_dirs[0] / "site.yaml"
out_path = clean_site_yaml(combined_yaml_path)
print("Cleaned YAML written to:", out_path)
